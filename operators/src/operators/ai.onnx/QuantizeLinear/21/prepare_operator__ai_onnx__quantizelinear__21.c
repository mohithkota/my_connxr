//this file was generated by ../../../../../../../../connx/connx_ajit/scripts/onnx_generator/OperatorTemplate.py
#include "operator__ai_onnx__quantizelinear__21.h"
#include "tracing.h"
#include "utils.h"

operator_status
prepare_operator__ai_onnx__quantizelinear__21(
    node_context *ctx
)
{
    TRACE_ENTRY(1);

    TRACE_NODE(2, true, ctx->onnx_node);

    /* UNCOMMENT AS NEEDED */

    // Onnx__TensorProto *i_x = searchInputByName(ctx, 0);
    // Onnx__TensorProto *i_y_scale = searchInputByName(ctx, 1);
    // Onnx__TensorProto *i_y_zero_point = searchInputByName(ctx, 2);

    // TRACE_TENSOR(2, true, i_x);
    // TRACE_TENSOR(2, true, i_y_scale);
    // TRACE_TENSOR(2, y_zero_point, i_y_zero_point);

    // Onnx__AttributeProto *a_axis = searchAttributeNyName(ctx->onnx_node->n_attribute,ctx->onnx_node->attribute,"axis");
    // Onnx__AttributeProto *a_block_size = searchAttributeNyName(ctx->onnx_node->n_attribute,ctx->onnx_node->attribute,"block_size");
    // Onnx__AttributeProto *a_output_dtype = searchAttributeNyName(ctx->onnx_node->n_attribute,ctx->onnx_node->attribute,"output_dtype");
    // Onnx__AttributeProto *a_saturate = searchAttributeNyName(ctx->onnx_node->n_attribute,ctx->onnx_node->attribute,"saturate");

    // TRACE_ATTRIBUTE(2, a_axis, a_axis);
    // TRACE_ATTRIBUTE(2, a_block_size, a_block_size);
    // TRACE_ATTRIBUTE(2, a_output_dtype, a_output_dtype);
    // TRACE_ATTRIBUTE(2, a_saturate, a_saturate);

    // Onnx__TensorProto *o_y = searchOutputByName(ctx, 0);

    /* ALLOCATE AND INITIALIZE CONTEXT HERE IF NEEDED */

    

    // context_operator__ai_onnx__quantizelinear__21 *op_ctx = NULL;
    // op_ctx = malloc(sizeof(context_operator__ai_onnx__quantizelinear__21));
    // TRACE_FATAL(0 , !op_ctx, "could not allocate executer_context");

    

    TRACE_VAR(2, true, op_ctx->axis, "%" PRId64);
    TRACE_VAR(2, true, op_ctx->block_size, "%" PRId64);
    TRACE_VAR(2, true, op_ctx->output_dtype, "%" PRId64);
    TRACE_VAR(2, true, op_ctx->saturate, "%" PRId64);

    /* INITIALIZE OUTPUTS DATA_TYPE AND SHAPE HERE */


    /* MALLOC OUTPUT TENSORS HERE */

    // mallocTensorData(o_y);

    // TRACE_TENSOR(2, true, o_y);

    /* CHOOSE EXECUTER AND CONTEXT HERE */
    /* YOU MAY USE THE GENERATED RESOLVER */

    // ctx->executer = resolve_operator__ai_onnx__quantizelinear__21(ctx);
    // ctx->executer_context = op_ctx;

    TRACE_EXIT(1);

    /* CHANGE RETURN CODE IF THIS PREPARER IS VALID */
    return OP_ENOSYS;
    // return OP_OK;
}