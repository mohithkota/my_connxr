...
# ifndef OPERATOR_OPERATOR__AI_ONNX__CONVTRANSPOSE__22_H
# define OPERATOR_OPERATOR__AI_ONNX__CONVTRANSPOSE__22_H
...
/**
 * ai.onnx operator 'ConvTranspose' version 22
 *
 ...
 *   or SAME_LOWER mean pad the input so that `output_shape[i] = input_shape[i]
 *   * strides[i]` for each axis `i`. The padding is split between the two
 *   sides equally or almost equally (depending on whether it is even or odd).
 *   In case the padding is an odd number, the extra padding is added at the
 *   end for SAME_UPPER and at the beginning for SAME_LOWER.
 * 
 ...
 *   ignored. See doc for details for equations to generate pads. Note that the
 *   output_shape attribute value should not include dimensions for batch size
 *   and channels, which are automatically inferred.
 * 
 ...
 * @since version 22
 *
 * @see github/workspace/onnx/defs/nn/defs.cc:1366
 * @see https://github.com/onnx/onnx/blob/master/docs/Operators.md#ConvTranspose
 */

operator_status
prepare_operator__ai_onnx__convtranspose__22(
    node_context *ctx
);

extern operator_info info_operator__ai_onnx__convtranspose__22;

typedef struct {
// no attributes
} context_operator__ai_onnx__convtranspose__22;

operator_executer
resolve_operator__ai_onnx__convtranspose__22(
    node_context *ctx
);

operator_status
execute_operator__ai_onnx__convtranspose__22(
    node_context *ctx
);

operator_status
execute_operator__ai_onnx__convtranspose__22__T_tensor_bfloat16(
    node_context *ctx
);

operator_status
execute_operator__ai_onnx__convtranspose__22__T_tensor_double(
    node_context *ctx
);

operator_status
execute_operator__ai_onnx__convtranspose__22__T_tensor_float(
    node_context *ctx
);

operator_status
execute_operator__ai_onnx__convtranspose__22__T_tensor_float16(
    node_context *ctx
);
