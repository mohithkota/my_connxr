...
if (ctx->inputs[0]) {
    ...
    case ONNX__TENSOR_PROTO__DATA_TYPE__BOOL: { executer = (operator_executer) &execute_operator__ai_onnx__if__19__B_tensor_bool; break; }
    default: {
        fprintf(stderr, "no matching type for operator__ai_onnx__if__19 and constraint 'B' with type '%s' found!\n",operator_info_tensorType2str(B));
        break;
    }
}
}
